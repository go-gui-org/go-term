...
type Cfg struct {
	...
	// starts once Close has started. A call that is still running is not
	...
	// Close runs on the main thread, so OnDownload must not wait for work it
	// queues there (QueueCommand, a dialog): that work cannot run until Close
	// returns, and every close then costs the full wait.
	//
